 ...
 */
function generateDemoComponentFiles(demo: DemoComponent, componentFolderPath: string): string[] {
  const generatedFiles: string[] = [];
  
  // Generate a file for each chunk
  demo.chunks.forEach(chunk => {
    const componentPath = `${componentFolderPath}/${demo.name.toLowerCase()}.${chunk.ext}`;
...
`;
    const componentPath = `${componentFolderPath}/${demo.name.toLowerCase()}.js`;
    const fullPath = path.join(process.cwd(), componentPath);
    const dir = path.dirname(fullPath);
 ...
 */
function getComponentImportPath(pageUrl: string, componentName: string, ext: string = 'gjs'): string {
